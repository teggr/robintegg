...
At the framework level, there are already plenty of Java SDKs operating at the agent harness layer with common primitives: LLM access, skills, agents, tools, and prompts. These are the building blocks. But I'm also watching an emergent category that operates at a higher level of abstraction — frameworks like [Embabel](https://github.com/embabel/embabel-agent) and now Kook (a newer entry in the same space), which are positioned around the concept of a **goal** rather than a workflow step. The framing shifts from "what does the agent do next" to "what is the agent trying to achieve." That's a meaningful change in perspective.
